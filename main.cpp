...
int main (int argc, char *argv[]) {
    ...
    while (!fin_inputs.eof()) {            
        ...
        if (strcmp(input_name.c_str(),"seed")==0) {
            ...
            fin_inputs >> m;
            input_cntr++;
        } else if (strcmp(input_name.c_str(),"N_cap")==0) {
            fin_inputs >> N_cap;
            N = N_cap;
            input_cntr++;
...
//    long progeny = 4*7*40*(n_h-m)/(19*m);
   ...
   FILE* fout_MC_seqs;
   {
    ...
    std::vector< std::vector<int8_t> > population(N_cap, std::vector<int8_t>(m));
    std::vector< std::vector<int8_t> > temp_pop(progeny*N_cap, std::vector<int8_t>(m));
    double* boltzfac = new double[N_cap];
    double* temp_boltz = new double[N_cap*progeny];
    double* part_sum = new double[N_cap*progeny];
    double Z = 1.0;
    double Ztemp = 1.0;
    double* boltzfac_eff = new double[N_cap];
    double Z_eff = 1.0;
    double* E = new double[N_cap];
    double* E_eff = new double[N_cap];
    double E_ave = 0.0;
    double E_eff_ave = 0.0;
    int* pop_init = new int[m];
    bool* occupied = new bool[N_cap];
    bool* occupied_temp = new bool[N_cap*progeny];
    int* idx = new int[N_cap*progeny];
    ...
    #pragma omp parallel default(shared) shared(nRes, resIdx, h, J, T, population, temp_pop, temp_boltz, m, N_cap, N, boltzfac, boltzfac_eff, Z, Z_eff, Ztemp, fout_P1_traj, fout_P2_traj, fout_MC_seqs, fout_pop_stats, fout_Tcell_traj, write_mod, print_mod, sample_mod, n_samples, cout, cerr, n_cycles, n1, n2, burnin, progeny, rate, seed, safeUnity, pos, Tcells, Etemp, n_T, chi, chiI, Ichi, T_penalty, n_WTepitopes, n_epitope, epi_start, epi_end, rep_lim, occupied, occupied_temp, idx, part_sum, place_value, E, E_ave, E_eff, E_eff_ave)
    {
    ...
    for (long cycle=1; cycle<=n_cycles; cycle++) {
            ...
            for(long i=begin; i<end; i++){   // loop over the processors part of the population
                if(occupied[i]){
                    ...
                    for(long j=0; j<m; j++){     // copy parent sequence
                        temp_pop[i+(N_cap*k)][j] = population[i][j];
                    }
                    for(long j=0; j<ranInt1; j++){ // introduce mutations
                        ...
                        if(temp_pop[i+(N_cap*k)][site] <= res) {
                            res++;
                        }
                        temp_pop[i+(N_cap*k)][site] = res;
                    }
                    idx[i+(N_cap*k)] = i+(N_cap*k);
                    double penalty = 0;
                    for(long j=0; j<n_epitope; j++){ // calculate T-cell suseptibility
                        int index = 0;
                        int count = 0;
                        for(long position=epi_start[j]; position<epi_end[j]; position++){
                            index += place_value[j][count]*temp_pop[i+(N_cap*k)][position];
                            count++;
                        }
                        double num_Ecells = 0;
                    ...
                    temp_boltz[i+(N_cap*k)] = exp(-(energy(temp_pop[i+(N_cap*k)],m,h,J))/T-penalty/T);
                    Ztemp_local += temp_boltz[i+(N_cap*k)];
                    occupied_temp[i+(N_cap*k)] = true;
                }
            }
        }
        ...
        #pragma omp for
        for(long i=0; i<N_cap*progeny; i++){
            if(occupied_temp[i]){
                ranNum1 = u(r)*safeUnity;
                if(ranNum1 < temp_boltz[i]/(temp_boltz[i]+Ztemp)){
                    occupied_temp[i] = true;
                    #pragma omp atomic
                        N++;
                } else {
                    occupied_temp[i] = false;
                }
            }
        }
    ...
    for(int i=0; i<n_epitope; i++){
     ...
     for(long i=0; i<n_epitope; i++){
        ...
        if(rep_lim < 2){ // effector generations equal 1
            ...
            integrate(int_Tcell_N, Tcells[i], 0.0, 7.0, 0.05);
        }
